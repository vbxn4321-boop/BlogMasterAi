...
[FORMATTING RULES]
${userFormattingPrompt || '본문 중간중간 가장 중요한 핵심 단어 1~2개만 매우 절제하여 강조하세요.'}
...
- **볼드([B]) 강조 사용 규칙 (절대 남발 금지 & 자연스러운 가독성)**:
  - **문장 전체 및 서술어 볼드 처리 엄격 금지**: 문장 전체나 '첫째', '~가능해요', '~주시거든요', '~도움이 돼요' 같은 서술어/접속어에 [B] 태그를 감싸지 마세요.
  - **단락당 1~2개 이하 핵심 단어구만 아주 절제하여 강조**: 각 단락에서 정말 중요한 핵심 명사어구(2~4단어 이내)에만 [B] 태그를 감싸세요.
    - **잘못된 예시 (절대 금지)**: \`[B]첫째, 다양한 정보 교환이 가능해요. 뜨개질 방법부터 실, 도구 추천까지 얻을 수 있어요.[/B]\`
    - **올바른 예시**: \`첫째, 다양한 정보 교환이 가능해요. [B]뜨개질 도안 추천[/B]부터 핵심 노하우까지 얻을 수 있거든요.\`
  - **[B] 태그 닫기 짝 맞춤 필수**: 모든 [B] 태그는 반드시 바로 뒤에 [/B] 닫는 태그를 정확히 짝을 맞춰 작성하세요. [B]만 혼자 출력되는 일이 없어야 합니다.
  - **원고 전체에서 3~5개 적정 유지**: 한 포스팅 전체에서 [B] 강조는 3~5개 이하로 아주 절제하여 명확하게 배치하세요.
