    ...
    return (
        <div style={{
            ...
            <div style={{ fontSize: 12.5, color: 'var(--text-secondary)', marginBottom: 10 }}>
                <div>🎯 핵심 키워드: <strong style={{ color: 'var(--text-primary)' }}>{proposal.main_keyword}</strong>
                    {proposal.volume != null && <span style={{ color: 'var(--text-muted)' }}> (월 {proposal.volume.toLocaleString()}회)</span>}
                </div>
                {proposal.sub_keywords?.length > 0 && (
                    <div style={{ marginTop: 4 }}>🔖 서브 키워드: {proposal.sub_keywords.join(', ')}</div>
                )}
            </div>
            <button onClick={() => onConfirm(proposal)} className="btn-primary" style={{ padding: '8px 16px', fontSize: 12.5 }}>
                이 키워드로 포스팅 시작 →
            </button>
        </div>
    );
...
export default function KeywordChatBox() {
    ...
    const goToPost = (proposal) => {
        const params = new URLSearchParams({
            ...
            sub_keywords: (proposal.sub_keywords || []).join(', '),
        });
